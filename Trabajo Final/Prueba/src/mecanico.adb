package body Mecanico is
   ...
   function Crearmecanico( nombre, apellido, especialidad: string; dni: natural ) return Tipomecanico is
      M: Tipomecanico;
   ...
   procedure Modificarmecanico(M: in out Tipomecanico; nuevosDatos: tipomecanico; rta: integer) is
   begin
      if Rta = 1 then --MODIFICAR NOMBRE 
         M.Nombre:= Nuevosdatos.Nombre;
         m.nombrelen:= nuevosDatos.nombreLen;
      else if Rta = 2 then
         M.apellido:= Nuevosdatos.apellido;
         m.apellidolen:= nuevosDatos.apellidoLen;         
      else
         M.apellido:= Nuevosdatos.apellido;
         M.Apellidolen:= Nuevosdatos.Apellidolen;
         M.nombre:= Nuevosdatos.nombre;
